...
namespace PickItUp.Behaviors
{
    ...
    {
        private class AgentActionCache
        {
            public float LastCheckTime;
            public bool IsExecutingAction;
        }

        private static readonly Dictionary<Agent, AgentActionCache> _actionCache = new Dictionary<Agent, AgentActionCache>();
        
        private static readonly HashSet<string> _cinematicActions = new HashSet<string>
        {
        ...
        public static bool IsExecutingCinematicAction(Agent agent)
        {
            ...
            {
                _actionCache.Remove(agent);
                return false;
            }

            float currentTime = Mission.Current.CurrentTime;

            if (_actionCache.TryGetValue(agent, out var cache))
            {
                if (currentTime - cache.LastCheckTime < 0.5f)
                {
                    return cache.IsExecutingAction;
                }
            }
            else
            {
                cache = new AgentActionCache();
                _actionCache[agent] = cache;
            }
            ...
            cache.IsExecutingAction = _cinematicActions.Contains(action0) || _cinematicActions.Contains(action1);
